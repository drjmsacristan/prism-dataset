...
1. **Representational efficiency.** On a sample-matched, controlled
...
2. **Quantified structural content.** A formal separability analysis
...
```bash
# Separability analysis (FDR / MI / ΔI) and all figures are produced by the
# single pipeline script:
python analysis/prism_pipeline.py        # -> results/prism_results.json, figures/*.png
# Per-position KS-entropy test (separate script):
python analysis/ks_entropy_profile.py    # -> results/ks_entropy_results.csv
...
analysis/        prism_pipeline.py (FDR/MI/ΔI + figures), ks_entropy_profile.py
results/         JSON/CSV outputs reproduced by the scripts above
data/            (matrices/models live on Zenodo; see data/README.md)
figures/         generated publication figures
SECURITY.md      responsible-use notice (no binaries are distributed)
```

## Data availability

- **PRISM matrices and trained baseline models:** Zenodo, DOI
  [10.5281/zenodo.20480349](https://doi.org/10.5281/zenodo.20480349).
...
- **Code:** [MIT](LICENSE)
- **Dataset / feature matrices:** [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/)
